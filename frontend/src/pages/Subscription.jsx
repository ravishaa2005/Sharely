    ...
    const handlePurchase = async (plan) => {
        ...
        try {
            ...
            const response = await axios.post(apiEndpoints.CREATE_ORDER, {
                planId: plan.id,
                amount: plan.price * 100, // still keep in paise for consistency
                currency: "INR",
            ...
            const orderId = response.data.orderId || "mock-order-id";

            // 🔹 Simulate success for now (you can toggle manually)
            const simulateSuccess = true;

            const verifyResponse = await axios.get(
                simulateSuccess
                    ? apiEndpoints.PAYMENT_SUCCESS(orderId)
                    : apiEndpoints.PAYMENT_FAILURE(orderId),
                { headers: { Authorization: `Bearer ${token}` } }
            );
